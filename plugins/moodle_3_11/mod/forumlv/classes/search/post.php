...
class post extends \core_search\base_mod {
    ...
    /**
     * Returns recordset containing required data for indexing forumlv posts.
     *
    ...
    public function get_recordset_by_timestamp($modifiedfrom = 0) {
        ...
        $sql = 'SELECT fp.*, f.id AS forumlvid, f.course AS courseid
                  FROM {forumlv_posts} fp
                  JOIN {forumlv_discussions} fd ON fd.id = fp.discussion
                  JOIN {forumlv} f ON f.id = fd.forumlv
    ...
    public function get_document($record, $options = array()) {
        ...
        try {
            $cm = $this->get_cm('forumlv', $record->forumlvid, $record->courseid);
    ...
    /**
     * Add the forumlv post attachments.
     *
    ...
    public function attach_files($document) {
        ...
        $cm = $this->get_cm('forumlv', $post->forumlv, $document->get('courseid'));
        $context = \context_module::instance($cm->id);

        // Get the files and attach them.
        $fs = get_file_storage();
        $files = $fs->get_area_files($context->id, 'mod_forumlv', 'attachment', $postid, "filename", false);
    ...
    public function check_access($id) {
        ...
        try {
            ...
            $forumlv = $this->get_forumlv($post->forumlv);
            $discussion = $this->get_discussion($post->discussion);
            $cminfo = $this->get_cm('forumlv', $forumlv->id, $forumlv->course);
    ...
    /**
     * Link to the forumlv post discussion
     *
    ...
    public function get_doc_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/forumlv/discuss.php', array('d' => $post->discussion));
    }
    ...
    public function get_context_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/forumlv/view.php', array('id' => $contextmodule->instanceid));
    }

    /**
     * Returns the specified forumlv post from its internal cache.
     *
     * @throws \dml_missing_record_exception
     * @param int $postid
     * @return stdClass
    ...
    protected function get_post($postid) {
        if (empty($this->postsdata[$postid])) {
            $this->postsdata[$postid] = forumlv_get_post_full($postid);
            if (!$this->postsdata[$postid]) {
                throw new \dml_missing_record_exception('forumlv_posts');
            }
        }
    ...
    /**
     * Returns the specified forumlv checking the internal cache.
     *
     ...
     * @return stdClass
     */
    protected function get_forumlv($forumlvid) {
        global $DB;

        if (empty($this->forumlvsdata[$forumlvid])) {
            $this->forumlvsdata[$forumlvid] = $DB->get_record('forumlv', array('id' => $forumlvid), '*', MUST_EXIST);
        }
        return $this->forumlvsdata[$forumlvid];
    }
    ...
    protected function get_discussion($discussionid) {
        ...
        if (empty($this->discussionsdata[$discussionid])) {
            $this->discussionsdata[$discussionid] = $DB->get_record('forumlv_discussions',
