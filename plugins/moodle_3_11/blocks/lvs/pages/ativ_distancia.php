...
if (! $cursolv->getConfiguracao()) {
	redirect(
		"$CFG->wwwroot/blocks/lvs/pages/config_cursolv.php?curso=$course->id", 
...
if (!isset($_POST['Salvar'])) {
	...
	if (count($foruns) != 0 || count($wikis) != 0) {
		$table->head = array(
			'Nome',
	...
	foreach ($foruns as $forum) {
		...
		$porcentagem = html_writer::empty_tag(
			'input',
			array(
				'type'=>'text',
				'id'=>"porcForum$i",
				'name'=>"atividade[forumlv][$i][porcentagem]",
				'value'=>$forum->porcentagem,
				'style'=>'width:40px'
			)
		);
		$porcentagem .= '%<br />';

		$exibir = html_writer::checkbox(
			"atividade[forumlv][$i][exibir]", 1, $forum->exibir
		);
		$exibir .= "<br />";
		...
		$etapa = html_writer::select(
			$options, "atividade[forumlv][$i][etapa]", $forum->etapa
		);
		$etapa .= "<br/>";

		$icons = html_writer::empty_tag(
			'span', array('class'=>'commands')
		);
		$icons .= html_writer::empty_tag(
			'a', array(
				'class'=>'editing_update',
				'title'=>$strupdate,
				'href'=>"$path/mod.php?update=$cm->id&sesskey=".sesskey()
			)
		);
		$icons .= html_writer::empty_tag(
			'img', array(
		...
		$icons .= html_writer::empty_tag(
			'a', array(
				'href'=>"$path/mod.php?delete=$cm->id&sesskey=".sesskey(),
				'onclick'=>"return confirm('Deseja realmente excluir essa atividade ?');"
			)
		);
		$icons .= html_writer::empty_tag(
			'img', array(
				'src'=> $OUTPUT->pix_url("/t/delete"),
				'class'=>'iconsmall', 'alt'=>"$strdelete"
			)
		);
		$table->data[] = array(
			$forum->name,
	...
	foreach ($wikis as $wiki) {
		...
		$porcentagem = html_writer::empty_tag(
			...
			array(
				'type'=>'text',
				'id'=>"porcWiki$j",
				'name'=>"atividade[wikilv][$j][porcentagem]",
				'value'=>$wikis_configuracoes[$wiki->id]->porcentagem,
				'style'=>'width:40px'
			)
		);
		$porcentagem .= '%<br />';
		...
		$exibir = html_writer::checkbox(
			"atividade[wikilv][$j][exibir]",
			1,
			$wikis_configuracoes[$wiki->id]->exibir
		);
		$exibir .= "<br />";
		...
		$etapa = html_writer::select(
			$options,
			"atividade[wikilv][$j][etapa]",
			$wikis_configuracoes[$wiki->id]->etapa
		);
		$etapa .= "<br/>";
	...
	*/
	...
	redirect(
		"$CFG->wwwroot/blocks/lvs/pages/ativ_distancia.php?curso=$course->id", 
		get_string('alteracoes', 'block_lvs')
	);
}