...
class DistanciaController {
	...
	/**
	 * 	Responsavel por acoes genericas a todos os controllers 
	 * 	@var AdapterController
	 */
	private $_adapterController;
	
	/**
	 * 	Recuperar dados ou persistir alteracoes nas atividades a distancia
	 * 	@var CursoLv
	 */
	...
	/**
	 * 	Exibe a pagina html referente às requisicoes
	 * 	@var DistanciaView
	 */
	...
	public function configurarAtividadesDistancia() {
		if (! $this->_cursolvModel->getConfiguracao() ) {
			$this->_adapterController->redirect(
				LVS_WWWROOT . '/pages/config_cursolv.php?curso=' . $this->_data['curso_ava']->id, 
...
// 		$gerenciadorTarefasLv = new TarefasLv($this->_cursolvModel);
		...
		if(!empty($this->_data['atividade'])) {
			...
			if (isset($this->_data['atividade']['forumlv'])) {
				$foruns = (array) Convert::array_to_object(
					$this->_data['atividade']['forumlv']
				);
...
// 			}
			...
			if (isset($this->_data['atividade']['chatlv'])) {
				$chats = (array) Convert::array_to_object(
					$this->_data['atividade']['chatlv']
				);
				$gerenciadorChatsLv->salvarConfiguracao($chats);
			}
			
			if (isset($this->_data['atividade']['wikilv'])) {
				$wikis = (array) Convert::array_to_object(
					$this->_data['atividade']['wikilv']
				);
				$gerenciadorWikisLv->salvarConfiguracao($wikis);
			}
			
			if (isset($this->_data['atividade']['quizlv'])) {
				$quizzes = (array) Convert::array_to_object(
					$this->_data['atividade']['quizlv']
				);
				$gerenciadorQuizzes->salvarConfiguracao($quizzes);
			}
			
			$this->_adapterController->redirect(
				LVS_WWWROOT . '/pages/atividades_distancia.php?curso=' . $curso_id,
...
// 		$this->_distanciaView->tarefas = $gerenciadorTarefasLv->recuperarConfiguracao($tarefas);
	...
	/**
	 * 	Altera os dados utilzados pelo controller no tratamento de requisicoes
	 * 	@param mixed $data
	 */
